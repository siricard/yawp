 ...
 * --no-bundle Reuse existing bundles in $MOOK_BUNDLE_DIR (CI: build
 * once, verify many).
 *
 * Exit codes:
...
const BUNDLE_DIR = process.env.MOOK_BUNDLE_DIR
  ? process.env.MOOK_BUNDLE_DIR
  : NO_BUNDLE
    ? null
    : mkdtempSync(join(tmpdir(), 'mook-verify-singletons-'));

if (NO_BUNDLE && !process.env.MOOK_BUNDLE_DIR) {
  console.error('verify-singletons: --no-bundle requires MOOK_BUNDLE_DIR to point at prebuilt bundles');
  process.exit(2);
}
