defmodule YawpWeb.AuthChannel do
  @moduledoc """
  The `auth:lobby` channel — the entry point for the Yawp authentication
  handshake.

  See `docs/` for the full wire format. In short:

    * On `join`, the server issues a fresh 32-byte random nonce (single-use,
      60s TTL) via `Yawp.Auth.NonceStore` and returns it base64-encoded in
      the join reply.
  ...
  def handle_in("authenticate", payload, socket) do
    with {:ok, _} <- Auth.validate_payload(payload),
         ...
         {:ok, _user} <- ensure_user(pubkey) do
                                          token =
        Phoenix.Token.sign(YawpWeb.Endpoint, YawpWeb.UserSocket.token_salt(), did,
          max_age: YawpWeb.UserSocket.token_max_age()
        )
