...
import {
  buildChannelReadMarkerCanonical,
  buildSendCanonical,
...
describe("signChannelReadMarker", () => {
  test("signs the same channel read marker envelope verified by the server", async () => {
    const pk = await ed.getPublicKey(SK);
    const signer = (bytes: Uint8Array): Uint8Array => ed.sign(bytes, SK);
    const ts = 1_700_000_000_000;
    const env = {
      channel_id: "11111111-1111-4111-8111-111111111111",
      identity_did: "did:yawp:zAlice",
      last_read_message_id: "m-1",
      ts,
    };
    const sigB64 = signChannelReadMarker(env, signer);
    expect(sigB64).toMatch(/^[A-Za-z0-9_-]+$/);
    expect(
      ed.verify(b64UrlToBytes(sigB64), buildChannelReadMarkerCanonical(env), pk)
    ).toBe(true);
    expect(new TextDecoder().decode(buildChannelReadMarkerCanonical(env))).toBe(
      `{"channel_id":"11111111-1111-4111-8111-111111111111","identity_did":"did:yawp:zAlice","last_read_message_id":"m-1","ts":${ts}}`
    );
  });
});
