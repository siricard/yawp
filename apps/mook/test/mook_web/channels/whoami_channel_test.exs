defmodule MookWeb.WhoamiChannelTest do
  @moduledoc """
  ...
  `lib/mook_web/channels/whoami_channel.ex`.
  """

  use MookWeb.ChannelCase, async: false

  alias MookWeb.UserSocket
  alias MookWeb.WhoamiChannel
  ...
  describe "end-to-end via UserSocket.connect" do
    test "valid token → whoami join succeeds with did" do
      token = Phoenix.Token.sign(MookWeb.Endpoint, "user_did", @did)
    ...
    test "expired token → whoami join fails with :unauthenticated" do
            token =
        Phoenix.Token.sign(MookWeb.Endpoint, "user_did", @did,
